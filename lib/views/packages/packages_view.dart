...
import '../widgets/shared/layouts/sub_page_header.dart'; // الهيدر الموحد
...
class _PackagesViewState extends State<PackagesView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            // استخدام الهيدر الموحد لتوفير الكود
            PremiumHeader(
              title: "إدارة باقات الشبكة",
              subtitle: "عدد الباقات المتوفرة: ${packages.length}",
            ...
            children: [
              ListTile(
                ...
                trailing: Text("${p.price} ريال",
                    style: const TextStyle(color: Color(0xFF1E3A8A), fontWeight: FontWeight.w900, fontSize: 18)),
              ),
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    _infoCell(Icons.timer_outlined, "الصلاحية", "${p.days}ي ${p.hours}س"),
                    _infoCell(Icons.data_usage_rounded, "البيانات", "${p.gigas}ج ${p.megas}م"),
  ...
  /* ---------------- SECTION LABEL (تم الحفاظ على التنسيق اليميني) ---------------- */
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _field(priceCtrl, "سعر البيع", Icons.payments_outlined, isNum: true),
              _sectionLabel("تحديد الرصيد"),
              Row(
                children: [
                  Expanded(child: _field(gigasCtrl, "جيجا بايت", Icons.storage_rounded, isNum: true)),
                  const SizedBox(width: 12),
                  Expanded(child: _field(megasCtrl, "ميجا بايت", Icons.sd_card_rounded, isNum: true)),
                ],
              ),
              _sectionLabel("تحديد الصلاحية"),
              Row(
                children: [
                  Expanded(child: _field(daysCtrl, "عدد الأيام", Icons.calendar_month, isNum: true)),
                  const SizedBox(width: 12),
                  Expanded(child: _field(hoursCtrl, "عدد الساعات", Icons.more_time, isNum: true)),
                ],
              ),
              _sectionLabel("تحديد الوقت"),
              _field(uptimeCtrl, "ساعات الارتباط (Uptime)", Icons.timer, isNum: true),
              _sectionLabel("تحديد السرعة"),
              _field(speedCtrl, "مثال: 4M/4M", Icons.speed),
  ...
  /* ---------------- TEXT FIELD (تم الحفاظ على التنسيق والاتجاه اليميني) ---------------- */
      ...
      child: Directionality(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(color: Color(0xFF1E3A8A), width: 1.5)),
          ),
        ),
      ),
    );
  ...
  Widget _saveButton(PackageItem? item, int? index) => InkWell(
        onTap: () {
          ...
          final p = PackageItem(
            ...
            days: int.tryParse(daysCtrl.text) ?? 0, hours: int.tryParse(hoursCtrl.text) ?? 0,
            gigas: int.tryParse(gigasCtrl.text) ?? 0, megas: int.tryParse(megasCtrl.text) ?? 0,
            uptimeLimit: uptimeCtrl.text, speedLimit: speedCtrl.text,
          );
          setState(() => item == null ? controller.addPackage(p) : controller.updatePackage(index!, p));
  ...
  Widget _buildActions(PackageItem p, int i) => Container(
        ...
        child: Row(
          ...
          children: [
            TextButton.icon(onPressed: () => _openUpsertSheet(item: p, index: i), icon: const Icon(Icons.edit_outlined), label: const Text("تعديل")),
            const VerticalDivider(),
            TextButton.icon(onPressed: () => setState(() => controller.deletePackage(i)), icon: const Icon(Icons.delete_outline, color: Colors.red), label: const Text("حذف", style: TextStyle(color: Colors.red))),
          ],
        ),
      );
  ...
  void _clearInputs() {
    ...
    gigasCtrl.clear(); megasCtrl.clear(); uptimeCtrl.clear(); speedCtrl.clear();
  }
}
